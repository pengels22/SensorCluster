...
# === Setup systemd service for watcher.py ===
echo "🛠 Setting up systemd service for watcher.py..."

WATCHER_SERVICE_NAME="watcher"
WATCHER_SERVICE_PATH="/etc/systemd/system/${WATCHER_SERVICE_NAME}.service"

sudo tee "$WATCHER_SERVICE_PATH" > /dev/null <<EOF
[Unit]
Description=SensorCluster Watcher Service
After=network.target

[Service]
ExecStart=/usr/bin/python3 $CLUSTER_DIR/Pi/watcher.py
WorkingDirectory=$CLUSTER_DIR/Pi
Restart=always
User=$USER
...
sudo systemctl daemon-reexec
sudo systemctl enable "$WATCHER_SERVICE_NAME"
